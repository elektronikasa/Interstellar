          ...
          <div class="ml-10 mb-4 mt-1">
            <div class="email__actions--single flex items-baseline">
              <feather-icon
                ...
                :svg-classes="[{'text-warning stroke-current stroke-current': currentMail.isStarred}, 'h-6 w-6']"
            ...
            >
              <div class="vx-col w-full">
                <vx-card class="px-4 mb-10">
                  ...
                  <div class="vx-row">
                    <div class="vx-col w-full border-b border-l-0 border-r-0 border-t-0 border-grey-light border-solid flex justify-between flex items-center">
                      <div class="flex items-center">
                        <div>
                          <vs-avatar
                            class="sender__avatar--single flex-no-shrink mr-3 border-2 border-solid border-white"
                            :src="require(`@/assets/images/portrait/small/${threadMail.img}`)"
                            size="65px"
                          />
                        </div>
                              ...
                              <vs-dropdown-menu style="z-index: 40001">
                                <div class="p-2">
                                  <p class="text-sm mb-1">
                                    From: <span class="font-semibold"> {{ threadMail.sender }} </span>
                                  </p>
                                  <p class="text-sm mb-1">
                                    To: <span class="font-semibold"> {{ threadMail.to | csv }} </span>
                                  </p>
                                  <p class="text-sm mb-1">
                                    Date: <span class="font-semibold"> {{ threadMail.time | time }} {{ threadMail.time | date(true) }} </span>
                                  </p>
                                  <p
                                    v-if="threadMail.cc.length"
                                    class="text-sm mb-1"
                                  >
                                    cc: <span class="font-semibold"> {{ threadMail.cc | csv }} </span>
                                  </p>
                                </div>
                              </vs-dropdown-menu>
                            </vs-dropdown>
                  ...
                  <!-- MAIL ATTACHMENTS -->
                  <div
                    v-if="threadMail.attachments.length"
                    class="vx-row"
                  >
                        ...
                        <div class="flex items-center">
                          ...
                          >
                            ...
                            <vs-dropdown-menu style="z-index: 40001">
                              <div class="p-2">
                                <p class="text-sm mb-1">
                                  From: <span class="font-semibold"> {{ currentMail.sender }} </span>
                                </p>
                                <p class="text-sm mb-1">
                                  To: <span class="font-semibold"> {{ currentMail.to | csv }} </span>
                                </p>
                                <p class="text-sm mb-1">
                                  Date: <span class="font-semibold"> {{ currentMail.time | time }} {{ currentMail.time | date(true) }} </span>
                                </p>
                                <p
                                  v-if="currentMail.cc.length"
                                  class="text-sm mb-1"
                                >
                                  cc: <span class="font-semibold"> {{ currentMail.cc | csv }} </span>
                                </p>
                              </div>
                            </vs-dropdown-menu>
                          </vs-dropdown>
    ...
    currentMailLabels: {
      ...
      set (value) {
        if (Array.isArray(value)) {
          const payload = { mailId: this.openMailId, value }
