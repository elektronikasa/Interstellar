...
<template>
  ...
  >
    ...
    <div class="mt-5">
      ...
      />
      <vs-list class="mt-5">
        <vs-list-item :title="` Heading: ${ pov && pov.heading }`" />
        <vs-list-item :title="` Pitch: ${ pov && pov.pitch }`" />
        <vs-list-item :title="` Pano ID: ${ pano }`" />
      </vs-list>
      <div class="demo-alignment">
        <vs-button
          type="filled"
          @click="pov.heading = Math.random() * 360"
        >
          Change Heading
        </vs-button>
        <vs-button
          type="filled"
          @click="pov.pitch = Math.random() * 180 - 90"
        >
          Change Pitch
        </vs-button>
        <vs-button
          type="filled"
          @click="pov.pitch = Math.random() * 180 - 90, pov.heading = Math.random() * 360"
        >
          Change Both
        </vs-button>
      </div>
