  ...
  <div class="vx-auto-suggest">
    <div class="flex items-center relative">
      <vs-input
        ...
        icon="icon-search"
        icon-no-border
@keyup.esc="escPressed" @keyup.up="increaseIndex(false)" @keyup.down="increaseIndex" @keyup.enter="suggestionSelected" @focus="updateInputFocus" @blur="updateInputFocus(false)"
      />
    </div>
    <ul
      ref="scrollContainer"
  ...
  watch: {
    ...
    searchQuery (val) {
      if (val == '') {
        ...
        const containEle = this.data.data.filter(item => !item.label.toLowerCase().startsWith(this.searchQuery.toLowerCase()) && item.label.toLowerCase().indexOf(this.searchQuery.toLowerCase()) > -1)
